...
let persons = [792907677737389]


module.exports = {
  // 970584629799490  486601201848598  717018355339492
  handleMessages:async function(messages, sender) {
    ...
    for (var i = 0; i < messages.length; i++) {
      if (
        ...
        setTimeout(
         await  self.handleCardMessages.bind(null, cardTypes, sender),
          timeout
        );
        cardTypes = [];
  ...
  handleMessageAttachments: function(messageAttachments, senderID) {
    ...
    self.sendTextMessage(senderID, "Attachment received. Thank you. fb");
  },
   ...
   *
   */
  sendImageMessage: async function(recipientId, imageUrl) {
    let self = module.exports;
    var messageData = {
      recipient: {
        id: recipientId
   ...
   await request(
      ...
      function(error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          if (messageId) {
            console.log(
              "Successfully sent message to recipient %s",
              recipientId
            );
          } else {
            console.log(
